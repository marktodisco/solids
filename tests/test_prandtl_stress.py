...
class TestPrandtlStress(unittest.TestCase):
    ...
    def test_stress_equals_gradient_of_phi(self):
        self.grad = [self.ps.phi.diff(symbol) for symbol in [self.x, self.y]]
        self.assertTrue(self.ps.sigma_xz.equals(self.grad[1]))
        self.assertTrue(self.ps.sigma_yz.equals(-self.grad[0]))
